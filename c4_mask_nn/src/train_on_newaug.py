...
if __name__ == '__main__':
    ...
    #load data
    log = '../log/' + time.strftime('%Y%m%d-%H%M%S')+'_v4_newaug/'
    backbone = 'vgg'
    if backbone == 'vgg':
    ...
    retrain = False
    ckpt_path = '../log/20200103-120258_v4_newaug/weight_0130.ckpt'

    #comofod数据集
    image_path = '../data/CoMoFoD_small'
    x_list, y_list = filter_image(image_path)
    #创建测试训练集
    test_xy = my_generator(x_list[::25], y_list[::25], batchs, image_size)
    ...
    train_xy = my_generator(x_list, y_list, batchs, image_size)
    print('data load done')

    #定义tensorboard回调可视化
    TBCallback = TensorBoard(log_dir=log)
    cpCallback = keras.callbacks.ModelCheckpoint(filepath=os.path.join(log, 'weight_{epoch:04d}.ckpt'), period=1)
    my_model = creat_my_model_newaug([image_size, image_size, 3], backbone=backbone, pre_weight_path=pre_weight_path, mode='train')
    my_model.summary()
